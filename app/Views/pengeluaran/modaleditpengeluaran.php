    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h5 class="modal-title text-white" id="modaleditpengeluaranLabel">Edit Pengeluaran</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <?= form_open('pengeluaran/update_pengeluaran') ?>
            <div class="modal-body">
                <div class="form-group">
                    <input type="hidden" name="id_pengeluaran" value="<?= $id_pengeluaran ?>">
                    <label for="exampleFormControlInput1">Nama Produk</label>
                    <input type="text" class="form-control" name="nama_produk" placeholder="Nama Produk" value="<?= $nama_produk ?>" required>
                </div>

                <div class="form-group">
                    <label for="exampleFormControlInput1">Jumlah Produk</label>
                    <input type="text" class="form-control" name="jumlah_produk" id="jumlah_produk" placeholder="Jumlah Produk" value="<?= $jumlah_produk ?>" required>
                </div>

                <div class="form-group">
                    <label for="exampleFormControlInput1">Harga Produk</label>
                    <input type="text" class="form-control numeric" name="harga_produk" id="harga_produk" value="<?= $harga_produk ?>" required>
                </div>

                <div class="form-group">
                    <label for="exampleFormControlInput1">Jumlah Pengeluaran</label>
                    <input type="text" class="form-control numeric" name="jumlah_pengeluaran" id="jumlah_pengeluaran" value="<?= $jumlah_pengeluaran ?>" readonly>
                </div>
            </div>
